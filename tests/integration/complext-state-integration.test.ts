...
interface ComplexState {
  ...
  updateNotificationFrequency: (frequency: number) => void;
  updateNotificationEnabled: (enabled: boolean) => void;
}

const initializer: StateCreator<ComplexState, [], []> = set => ({
  items: [],
  todos: {},
  users: {},
  settings: {
  ...
  addItem: (name: string) =>
    set(state => ({
      items: [...state.items, { id: `${Math.random().toString(36).substring(2, 15)}`, name }],
    })),
  ...
  updateNotificationFrequency: (frequency: number) =>
    set(state => ({
      settings: {
        ...state.settings,
        notifications: { ...state.settings.notifications, frequency },
      },
    })),
...
describe('Multiplayer Middleware Complex State Tests', () => {
  ...
  describe('Record Type Support', () => {
    ...
    it('should sync adding new records with ZFactor 0', async () => {
      const uniqueNamespace = createUniqueStoreName('record-sync-test');
    ...
    it('should sync adding new records with ZFactor 1', async () => {
      const uniqueNamespace = createUniqueStoreName('record-sync-test');
    ...
    it('should sync adding new records with zFactor 3', async () => {
      const uniqueNamespace = createUniqueStoreName('record-sync-test');
    ...
    it('should sync updating existing records with zFactor 0', async () => {
      const uniqueNamespace = createUniqueStoreName('record-sync-test');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 0 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 0 });
    ...
    it('should sync updating existing records with zFactor 1', async () => {
      const uniqueNamespace = createUniqueStoreName('record-sync-test');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
    ...
    it('should sync updating existing records with zFactor 3', async () => {
      const uniqueNamespace = createUniqueStoreName('record-sync-test');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
    ...
    it('should handle Record entry deletion with zFactor 0', async () => {
      const uniqueNamespace = createUniqueStoreName('record-delete-test');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 0 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 0 });
    ...
    it('should handle Record entry deletion with zFactor 1', async () => {
      const uniqueNamespace = createUniqueStoreName('record-delete-test');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
    ...
    it('should handle Record entry deletion with zFactor 2', async () => {
      const uniqueNamespace = createUniqueStoreName('record-delete-test');
    ...
    it('should delete the keys when the record entry is deleted', async () => {
      ...
      await waitFor(async () => {
        await expect(client.get(`${uniqueNamespace}-2:todos:1:id`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-2:todos:1:text`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-2:todos:1:completed`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-2:todos:2:id`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-2:todos:2:text`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-2:todos:2:completed`)).resolves.toHaveProperty(
          'code',
          200,
        );
      });
    ...
    it('should delete the keys when the record entry is deleted with zFactor 1', async () => {
      const uniqueNamespace = createUniqueStoreName('record-delete-test');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
      await waitFor(() => {
        expect(store1.getState().multiplayer.hasHydrated).toBe(true);
      ...
      await waitFor(async () => {
        await expect(client.get(`${uniqueNamespace}-1:todos:1`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-1:todos:2`)).resolves.toHaveProperty(
          'code',
          200,
        );
      });
    ...
    it('should delete the keys when the record entry is deleted with zFactor 3', async () => {
      const uniqueNamespace = createUniqueStoreName('record-delete-test');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 3 });
      await waitFor(() => {
        expect(store1.getState().multiplayer.hasHydrated).toBe(true);
      ...
      await waitFor(async () => {
        await expect(client.get(`${uniqueNamespace}-3:todos:1:id`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-3:todos:1:text`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-3:todos:1:completed`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-3:todos:2:id`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-3:todos:2:text`)).resolves.toHaveProperty(
          'code',
          200,
        );
        await expect(client.get(`${uniqueNamespace}-3:todos:2:completed`)).resolves.toHaveProperty(
          'code',
          200,
        );
      });
  ...
  describe('Nested Object Updates', () => {
    it('should synchronize nested object updates when zFactor is 0', async () => {
      const uniqueNamespace = createUniqueStoreName('namespace');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 0 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 0 });
    ...
    it('should synchronize nested object updates when zFactor is 1', async () => {
      const uniqueNamespace = createUniqueStoreName('namespace');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 1 });
    ...
    it('should synchronize nested object updates when zFactor is 2', async () => {
      const uniqueNamespace = createUniqueStoreName('namespace');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 2 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 2 });
    ...
    it('should synchronize nested object updates when zFactor is 3', async () => {
      const uniqueNamespace = createUniqueStoreName('namespace');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 3 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 3 });
    ...
    it('should synchronize nested object updates when zFactor is 4', async () => {
      const uniqueNamespace = createUniqueStoreName('namespace');
      const store1 = createTestStore({ namespace: uniqueNamespace, zFactor: 4 });
      const store2 = createTestStore({ namespace: uniqueNamespace, zFactor: 4 });
